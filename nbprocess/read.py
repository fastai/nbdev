...
# %% auto 0
__all__ = ['display_json', 'NbCell', 'dict2nb', 'read_nb', 'nbprocess_create_config', 'get_config', 'add_init', 'write_cells', 'basic_export_nb']
...
def write_cells(cells, hdr, file, offset=0):
    for cell in cells:
        # Don't include '#export' lines in exported code
        source = re.sub(r'^#\s*export[is]?\s*\n', '', cell.source)
        if source.strip():
