...
# %% auto 0
__all__ = ['mk_cell', 'create_output', 'CellRunner', 'strip_ansi', 'hide_', 'hide_line', 'filter_stream_', 'clean_magics',
           'lang_identify', 'rm_header_dash', 'rm_export', 'exec_show_docs', 'clean_show_doc', 'insert_warning',
           'get_title', 'add_show_docs']
...
def _get_src(nb, cell_type): return nb.cells.filter(lambda c: c.cell_type == cell_type).attrgot('source')

def get_title(nb): 
    "Get the title and description from a notebook from the H1"
    md = _get_src(nb, 'markdown').map(_re_title.match).filter()
    if md: return md[0].groups()
    else: return None,None

# %% ../nbs/09_processors.ipynb 44
def _is_fm(nb):
    "See if there is frontmatter in the notebook as a raw cell"
    return bool(_get_src(nb, 'raw').map(_re_fm.search).filter())
