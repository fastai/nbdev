...
# %% auto 0
__all__ = ['find_var', 'read_var', 'update_var', 'ModuleMaker', 'retr_exports', 'relative_import', 'absolute_import', 'update_import', 'basic_export_nb2']
...
def make_all(self:ModuleMaker, cells):
    ...
    parsed = cells.map(NbCell.parsed_).concat()
    return retr_exports(parsed)

# %% ../nbs/01_maker.ipynb 24
...
def import2relative(cell:NbCell, libname):
    src = update_import(cell.source, cell.parsed_(), libname, relative_import)
    if src: cell.set_source(src)

# %% ../nbs/01_maker.ipynb 30
@patch
...
def basic_export_nb2(fname, name, dest=None):
    ...
    mm = ModuleMaker(dest=dest, name=name, nb_path=fname)
    mm.make(cells)
