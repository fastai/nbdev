...
class NB(BaseNB):
    ...
    def __init__(self,
        ...
        if isinstance(nb, NB): nb = nb.nb
        elif isinstance(nb, (str, Path)): nb = read_nb(nb)
        if not isinstance(nb, AttrDict): raise TypeError(f'nb must be AttrDict, NB, or Path but found type: {type(nb)}')
        super().__init__(nb)
