...
# %% auto 0
__all__ = ['yaml_str', 'dict2fm', 'default_exp', 'yml2dict', 'NB', 'create_output', 'show_src', 'nbdev_create_config',
           'get_config', 'config_key', 'add_init', 'write_cells', 'basic_export_nb']
...
class NB(AttrDict):
    ...
    def __init__(self,
        ...
        if isinstance(nb, (str, Path)): nb = read_nb(nb)
        elif not isinstance(nb, AttrDict): raise TypeError(f'nb must be AttrDict, NB, or Path but found type: {type(nb)}')
        super().__init__(nb)
        self._raw_fm_dict_ = yml2dict(getattr(self._fm_cell_, 'source', None))

    @property
    def default_exp_(self): return default_exp(self)

    @property
    def text_(self): return concat(L(self.cells).attrgot('source').filter())
    
    def print_txt_(self, n=None): 
        for t in self.text_[:n]: print(t + '\n')

    @property
    def title_cell_(self): return _select_cell(self, 'markdown', _re_title.search)        
                                       
    @property
    def _fm_cell_(self): return _select_cell(self, 'raw', _re_fm.search)
    ...
    def _md_fm_dict_(self): 
        ...
        if not self.title_cell_: return {}
        title_match = _re_title.match(self.title_cell_.source or '')
        if title_match:
            title,desc=title_match.groups()
            flags = re.findall('^-\s+(.*)', self.title_cell_.source, flags=re.MULTILINE)
