...
__all__ = ['first', 'read_nb', 'check_re', 'is_export', 'find_default_export', 'export_names', 'extra_add',
           ...
           'create_mod_file', 'add_init', 'update_version', 'update_baseurl', 'notebook2script', 'DocsTestClass']
...
# Matches any line, with any kind of export flag, without any module name:
^%nbdev_                     # beginning of line (since re.MULTILINE is passed) and start of magic flag
export(|_and_show|_internal) # export, export_and_show or export_internal
...
# Matches any line, with any kind of export flag, with a module name and catches it in group 1:
^%nbdev_                       # beginning of line (since re.MULTILINE is passed) and start of magic flag
export(?:|_and_show|_internal) # export, export_and_show or export_internal (non-capturing group)
...
# Matches any line, with export internal flag, with or without module name:
^%nbdev_        # beginning of line (since re.MULTILINE is passed) and start of magic flag
...
def is_export(cell, default):
    "Check if `cell` is to be exported and returns the name of the module to export if provided"
...
# Matches any line, with default export, with a module name and catches it in group 1:
^%nbdev_       # beginning of line (since re.MULTILINE is passed) and start of magic flag
...
def _notebook2script(fname, silent=False, to_dict=None):
    "Finds cells starting with `%nbdev_export` and puts them into a new module"
