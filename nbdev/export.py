...
def create_mod_file(fname, nb_path):
    ...
    fname.parent.mkdir(parents=True, exist_ok=True)
    filename = os.path.relpath(nb_path, Config().config_file.parent).replace('\\', '/') #windows sep
    with open(fname, 'w') as f:
        f.write(f"# AUTOGENERATED! DO NOT EDIT! File to edit: {filename} (unless otherwise specified).")
        f.write('\n\n__all__ = []')
