...
#Cell
def export_nb(nb, tmp_f, orig_dir, flags=None):
    ...
    with open(tmp_f, 'w', encoding='utf8') as f: f.write(f"import os\nos.chdir('{orig_dir}')\n\n")
...
def test_nb(fn, flags=None, tmp_dir='.'):
    ...
    try:
        nb = read_nb(fn)
        export_nb(nb, tmp_f, fn.parent, flags=flags)
        if tmp_f.exists(): runpy.run_path(tmp_f)
    finally:
        #if tmp_f.exists(): tmp_f.unlink()
